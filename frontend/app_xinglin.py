# ── 杏林AI 医疗知识问答系统 前端 ──
# 设计决策：
# - 品牌名 杏林AI（XingLin）：杏林 = 中国医界雅称，与 LexAI 同系列产品线
# - 配色：深杏林绿(#14513b) + 主绿(#1f8a5f) + 薄荷绿(#34a06b)，洁净临床米白底(#f4f8f6)
# - 衬线字体 Noto Serif SC（标题）保持专业感，Inter（正文）保持清晰
# - 卡片式布局，引用来源 hover 高亮绿色边框
# - 新增「非医疗建议」常驻免责声明（医疗场景合规必需）
...
with st.sidebar:
    ...
    st.markdown("### 🩺 杏林AI")
    st.caption("Medical Intelligence System")
...
with col_title:
    st.markdown('<div class="hero-title">杏林AI 医疗知识问答系统</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-sub">RAG 增强 · 知识图谱 · 溯源可信 · 安全合规</div>', unsafe_allow_html=True)

# 常驻免责声明（医疗场景合规必需）
st.markdown(
    '<div class="disclaimer">'
    '⚠️ <strong>免责声明：</strong>本系统仅基于公开医学资料提供检索与参考，'
    '不构成医疗建议、诊断或处方。急症请立即就医或拨打 <strong>120</strong>；'
    '具体诊疗请前往正规医疗机构由执业医师决定。'
    '</div>',
    unsafe_allow_html=True
)
...
tab_doc, tab_qa = st.tabs(["📂 文档管理", "💡 医疗检索"])
...
with tab_doc:
    ...
    with col_left:
        st.markdown('<div class="card-title" style="border-bottom:none;margin-bottom:0.5rem;">📄 上传医学资料</div>', unsafe_allow_html=True)

        uploaded_files = st.file_uploader(
            "选择医学文档（PDF / Word，最多 5 份）",
            type=["pdf", "docx", "doc"],
        ...
        if uploaded_files:
            ...
            st.info("支持 PDF、DOCX 格式医学资料（临床指南 / 药品说明书），最多 5 份同时上传。", icon="ℹ️")
        ...
        with st.expander("📚 已入库文档", expanded=True):
            ...
            if documents:
                ...
                <div class="empty-state">
                    ...
                    暂无入库文档<br>
                    <span style="font-size:0.8rem;">请先在左侧上传医学资料</span>
                </div>
                """, unsafe_allow_html=True)
...
with tab_qa:
    st.markdown('<div class="card-title" style="border-bottom:none;margin-bottom:0.5rem;">🔍 医学知识检索</div>', unsafe_allow_html=True)
    ...
    question = st.text_input(
        "## 输入健康问题",
        placeholder="例：二甲双胍的适应症与常见不良反应有哪些？",
        label_visibility="collapsed",
    ...
    if st.session_state.last_result:
        ...
        if found:
            ...
            st.markdown(f'<div class="answer-box safe">⚠ {answer}<br><br>本内容基于公开医学资料生成，仅供学习参考，不构成医疗建议。</div>', unsafe_allow_html=True)
    ...
    # Footer
    st.markdown('<div class="footer-note">杏林AI Medical Intelligence · 严谨 · 专业 · 合规 · 仅供学习参考</div>', unsafe_allow_html=True)
